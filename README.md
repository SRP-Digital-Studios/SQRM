# SQRM - Saksham QR Menu

Smart Restaurant QR Menu & Ordering Platform

SQRM (Saksham QR Menu) is a modern QR-based restaurant menu and ordering platform that enables restaurants to create digital menus, receive customer orders, manage restaurants, track analytics, and generate QR codes without requiring customers to create accounts.

Developed and maintained by SRP Digital Studios.

---

## Features

### Restaurant Owner Features

* Google Sign-In Authentication
* Multi-Restaurant Management
* Restaurant Logo Upload
* QR Code Generation
* QR Code Download (PNG/PDF)
* Menu Management
* Category Management
* Order Management
* Customer Order Tracking
* Notification Center
* Analytics Dashboard
* CSV Export
* Excel Export
* Professional Bill & Receipt Generation

### Customer Features

* No Registration Required
* QR Menu Access
* Search Food Items
* Category Filtering
* Add To Cart
* Checkout System
* Order Tracking
* Restaurant Rating System
* Order Again Functionality

### Super Admin Features

* Platform Dashboard
* Restaurant Management
* Owner Management
* Plan Management
* Subscription Controls
* Platform Analytics
* Order Monitoring
* Phone Number Management

---

## Subscription Plans

### Free Plan

* 1 Restaurant
* 20 Menu Items Per Restaurant
* Daily Earnings Analytics
* Monthly Earnings Analytics
* Order History
* CSV / Excel Export

### Basic Plan

* Up To 5 Restaurants
* Unlimited Menu Items
* Detailed Monthly Analytics
* Best Selling Items
* Customer Insights
* Order History
* CSV / Excel Export

### Premium Plan

* Unlimited Restaurants
* Unlimited Menu Items
* Lifetime Analytics
* Advanced Reports
* Popular Categories
* Active Hour Analysis
* All Basic Features

---

## Technology Stack

### Backend

* PHP 8+
* MySQL

### Frontend

* HTML5
* CSS3
* JavaScript
* AJAX


---

## Core Modules

### Public Website

* Homepage
* Pricing Page
* Restaurant Leaderboards
* Restaurant Details

### Restaurant Management

* Create Restaurant
* Edit Restaurant
* Delete Restaurant
* Status Management

### Menu Management

* Add Menu Item
* Edit Menu Item
* Delete Menu Item
* Category Management

### Order Management

* Current Orders
* Previous Orders
* Order Status Updates
* Receipt Generation

### Analytics

* Revenue Tracking
* Order Statistics
* Customer Insights
* Product Performance

---


## Security

* Session Protection
* Owner Data Isolation
* Prepared SQL Statements
* Secure Authentication
* Image Validation
* Upload Restrictions

---

## Mobile Support

Optimized for:

* 320px
* 375px
* 414px
* 480px
* Tablets
* Desktop Devices

---

## SEO Features

* Meta Tags
* Open Graph Tags
* JSON-LD Structured Data
* Search Engine Friendly URLs

---

## Project Status

Current Status: Active Development

Version: v1.0.0

---

## Official Domain

https://sqrm.srpdigitalstudios.qzz.io

---

## Company

SRP Digital Studios

Developing modern web solutions, digital platforms, and business software.

---

## Copyright

Copyright © 2026 SRP Digital Studios.

All Rights Reserved.

This software and its source code are proprietary software owned by SRP Digital Studios.

Unauthorized copying, modification, distribution, resale, reverse engineering, or commercial use of this software is strictly prohibited without prior written permission from SRP Digital Studios.
